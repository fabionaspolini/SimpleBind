...
namespace SimpleBind.Core.Lib
{
    public static class ReflectionExtensions
    {
        public static ConstructorInfo GetConstructor(this Type aType, params Type[] aArgTypes)
        {
            ConstructorInfo lRet = null;
            var lConstructors = aType.GetTypeInfo().DeclaredConstructors.Where(p => p.GetParameters().Length == aArgTypes?.Length);
            foreach (var lCons in lConstructors)
            {
                lRet = lCons;
                var lParams = lCons.GetParameters();
                for (int i = 0; i < aArgTypes.Length; i++)
                    if (lParams[i].ParameterType != aArgTypes[i])
                    {
                        lRet = null;
                        break;
                    }
