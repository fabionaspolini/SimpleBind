...
{
    [TestFixture]
    public class NotifyPropertyChangedPersonConvertTest
    {
        private BindContainer<Person> _container;
        private Person _sourcePerson = new Person();
        private PersonConvert _destPerson;
        ...
        private void CreateBinds(BindOrientarion orientarion)
        {
            ...
            {
                var lBinded = (BindedItem<Person, PersonConvert>) lItem;
                switch (orientarion)
                {
        ...
        private void SetDestValues()
        {
            ...
            _destPerson.SalaryDecimal = 74123.32m.ToString(CultureInfo.InvariantCulture);
            _destPerson.SalaryDouble = 74123.32f.ToString(CultureInfo.InvariantCulture);
            _destPerson.SalaryFloat = 74123.32f.ToString(CultureInfo.InvariantCulture);
            _destPerson.SexChar = 'M'.ToString();
            _destPerson.SexEnum = Sex.Male.ToString();
            _destPerson.Nationality = (int) Nationality.USA;
        }

        private void AssertEqualsValues()
        {
            Assert.AreEqual(_sourcePerson.Id.ToString(), _destPerson.Id);
            Assert.AreEqual(_sourcePerson.Name, _destPerson.Name);
            Assert.AreEqual(_sourcePerson.Active.ToString(), _destPerson.Active);
            Assert.AreEqual(_sourcePerson.Age.ToString(), _destPerson.Age);
            Assert.AreEqual(_sourcePerson.AgeShort.ToString(), _destPerson.AgeShort);
            Assert.AreEqual(_sourcePerson.AccountNumber.ToString(), _destPerson.AccountNumber);
            Assert.AreEqual(_sourcePerson.BirthData.ToString(CultureInfo.InvariantCulture), _destPerson.BirthData);
            Assert.AreEqual(_sourcePerson.SalaryDecimal.ToString(CultureInfo.InvariantCulture), _destPerson.SalaryDecimal);
            Assert.AreEqual(_sourcePerson.SalaryDouble.ToString(CultureInfo.InvariantCulture), _destPerson.SalaryDouble);
            Assert.AreEqual(_sourcePerson.SalaryFloat.ToString(CultureInfo.InvariantCulture), _destPerson.SalaryFloat);
            Assert.AreEqual(_sourcePerson.SexChar.ToString(), _destPerson.SexChar);
            Assert.AreEqual(_sourcePerson.SexEnum.ToString(), _destPerson.SexEnum);
            Assert.AreEqual((int) _sourcePerson.Nationality, _destPerson.Nationality);
        }
        ...
        private void AssertNotEqualsValues()
        {
            ...
            Assert.AreNotEqual(_sourcePerson.SexEnum.ToString(), _destPerson.SexEnum);
            Assert.AreNotEqual((int)_sourcePerson.Nationality, _destPerson.Nationality);
        }
    }
}
